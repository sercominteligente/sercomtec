...
const state = {
  ...
  room: null,
  party: { enabled: false, intervalSeconds: 30, nextAt: 0, lastSpeaker: '' },
  agents: {},
  messages: [],
  busy: false,
  autoBusy: false,
...
const autopilotToggle = $('#autopilot-enabled');
const autopilotInterval = $('#autopilot-interval');
...
function setComposerPermission() {
  ...
  if (state.busy) {
    topicInput.placeholder = 'Algumas IAs pegaram o microfone...';
    ...
    topicInput.placeholder = 'Jogue uma frase, pergunta ou provocação na roda...';
  }
}

function renderAutoStatus() {
  if (!autoStatus) return;
  const enabled = Boolean(state.party?.enabled && state.room?.isOpen);
  autoStatus.classList.toggle('off', !enabled);
  autoStatus.classList.toggle('pulsing', enabled && state.autoBusy);
  const label = autoStatus.querySelector('b');
  if (label) label.textContent = enabled ? `Resenha automática · ~${state.party.intervalSeconds}s` : 'Resenha automática pausada';
  autoStatus.title = enabled
    ? 'Uma IA aleatória entra na conversa a cada intervalo, sem ordem fixa.'
    : 'O modo de conversa autônoma está pausado.';
}
...
function renderRoomState() {
  ...
  observerNote.querySelector('small').textContent = state.room.publicCanPrompt
    ? 'Visitantes podem jogar provocações na roda.'
    : 'O Super Admin decide quando o público pega o microfone.';
  ...
  $$('input[data-agent]', adminAgentList).forEach((checkbox) => {
    checkbox.addEventListener('change', async () => {
      ...
      if (selected.length < 2) {
        ...
        window.alert('Deixe pelo menos dois agentes na roda. Monólogo de robô é podcast ruim. 😄');
        return;
      }
...
function renderAdmin() {
  ...
  if (autopilotToggle) autopilotToggle.checked = Boolean(state.party?.enabled);
  if (autopilotInterval) autopilotInterval.value = String(state.party?.intervalSeconds || 30);
...
async function saveParty(patch) {
  ...
  try {
    const data = await api('/api/master/admin/party', {
      method: 'POST',
      body: JSON.stringify(patch)
    });
    state.party = data.party || state.party;
    renderAll();
    if (state.party.enabled) setTimeout(() => maybeAutoTick(), 2800);
  } catch (error) {
    window.alert(error.message);
    await loadState();
  }
...
async function submitRound(topic) {
  ...
  setComposerPermission();
  sendRound.querySelector('span').textContent = 'Soltando...';
  const thinkingCopy = thinkingBar.querySelector('span:last-child');
  if (thinkingCopy) thinkingCopy.textContent = 'O microfone está solto. Uma, duas ou três IAs podem responder em ordem aleatória...';

  try {
    await api('/api/master/party/prompt', {
      method: 'POST',
    ...
    sendRound.querySelector('span').textContent = 'Jogar na roda';
    setComposerPermission();
    topicInput.focus();
  }
}

async function maybeAutoTick(force = false) {
  if (state.autoBusy || state.busy || document.hidden) return;
  if (!force && (!state.party?.enabled || !state.room?.isOpen)) return;
  state.autoBusy = true;
  renderAutoStatus();
  try {
    const data = await api('/api/master/party/tick', {
      method: 'POST',
      body: JSON.stringify({ force })
    });
    if (data.party) state.party = data.party;
    if (data.generated) await loadState({ quiet: false, forceBottom: true });
    else renderAutoStatus();
  } catch (error) {
    if (force) window.alert(error.message);
  } finally {
...
autopilotToggle?.addEventListener('change', (event) => saveParty({ enabled: event.target.checked }));
autopilotInterval?.addEventListener('change', (event) => saveParty({ intervalSeconds: Number(event.target.value) || 30 }));

$('#continue-button').addEventListener('click', () => maybeAutoTick(true));

$('#clear-button').addEventListener('click', async () => {
  if (!window.confirm('Limpar todo o bate-papo da sala? As IAs vão fingir que nunca disseram nada. 😄')) return;
  try {
...
loadState({ forceBottom: true }).then(() => {
  if (state.party?.enabled) setTimeout(() => maybeAutoTick(), 1500);
});
...
setInterval(() => {
  if (!document.hidden) maybeAutoTick();
}, 6500);
